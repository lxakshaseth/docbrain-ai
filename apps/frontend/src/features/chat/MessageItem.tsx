      ...
      <div className={`max-w-2xl rounded-2xl p-4 text-sm ${
        ...
        {message.sources && message.sources.length > 0 && (
          <div className="mt-3 pt-3 border-t border-slate-800/80">
            <p className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 mb-2">
              <BookOpen className="w-3.5 h-3.5 text-blue-400" /> Grounded Source Citations:
            </p>
            <div className="flex flex-wrap gap-2">
              {message.sources.map((source, index) => (
                <div
                  key={index}
                  className="bg-slate-950/80 border border-slate-800 hover:border-blue-500/50 rounded-lg p-2 text-xs transition-colors"
                >
        ...
        {!isUser && suggestedQuestions.length > 0 && (
          <div className="mt-4 pt-3 border-t border-slate-800/60">
            <p className="text-[11px] font-semibold text-slate-400 flex items-center gap-1 mb-2">
              <HelpCircle className="w-3.5 h-3.5 text-indigo-400" /> Suggested Follow-up Questions:
            </p>
            <div className="space-y-1.5">
              {suggestedQuestions.map((q, idx) => (
                <button
                  ...
                  className="w-full text-left flex items-center justify-between p-2 rounded-lg bg-slate-950/60 hover:bg-blue-600/10 border border-slate-800 hover:border-blue-500/40 text-xs text-slate-300 hover:text-blue-300 transition-all group"
                >
                  <span>{q}</span>
