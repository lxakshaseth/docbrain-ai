...
export const ChatBox: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-950 rounded-2xl border border-slate-200/80 dark:border-slate-800/80 overflow-hidden shadow-xs transition-colors duration-250">
        ...
        <div className="max-w-7xl mx-auto space-y-4">
          {!activeDocument ? (
            <div className="h-full min-h-[380px] flex flex-col items-center justify-center text-center p-8">
              <div className="w-14 h-14 rounded-2xl bg-blue-50 dark:bg-blue-950/50 border border-blue-100 dark:border-blue-900/50 flex items-center justify-center mb-4 text-blue-600 dark:text-blue-400">
            ...
            <div className="min-h-[420px] flex flex-col items-center justify-center text-center p-6">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 flex items-center justify-center text-white mb-3 shadow-md">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md mt-1 mb-6">
                Ask any question about <span className="font-medium text-slate-700 dark:text-slate-300">"{activeDocument.title}"</span>.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 w-full max-w-4xl">
                {starterPrompts.map((prompt, idx) => (
                  <button
                    ...
                    className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 hover:border-blue-500/40 dark:hover:border-blue-500/40 text-left transition-all hover:shadow-xs group"
                  >
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 mb-1 group-hover:text-blue-600 dark:group-hover:text-blue-400">
          ...
          {isStreaming && (
            <div className="flex gap-3 justify-start my-4">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-white shrink-0 shadow-sm">
                <Loader2 className="w-4 h-4 animate-spin" />
              </div>
              <div className="bg-slate-100 dark:bg-slate-900 rounded-2xl rounded-tl-xs px-4 py-3 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2">
      ...
      <div className="p-4 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md shrink-0">
        <div className="max-w-7xl mx-auto">
          <form
            onSubmit={handleSubmit}
            className="flex items-center gap-2 p-1.5 rounded-2xl bg-slate-100/80 dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-800/80 focus-within:border-blue-500/50 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all shadow-xs"
          >
            <input
              type="text"
              value={inputQuery}
            ...
            <button
              ...
              className="w-9 h-9 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold shadow-xs transition-all disabled:opacity-40 disabled:hover:bg-blue-600 flex items-center justify-center shrink-0"
              title="Send message"
            >
              {isStreaming ? (
                <Loader2 className="w-4 h-4 animate-spin" />
