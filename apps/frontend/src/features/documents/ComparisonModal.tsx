      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-slate-950 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-gradient-to-tr from-emerald-600 to-teal-600 rounded-xl text-white">
              <GitCompare className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-white">Multi-Document Comparison Matrix</h3>
              <p className="text-xs text-slate-400">Comparing {selectedDocIds.length} selected documents</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Body */}
        <div className="flex-1 p-6 overflow-auto bg-slate-950/40 text-slate-200">
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-1">
                <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400">Synthesis Overview</h4>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-950 border-b border-slate-800">
                      <th className="p-3 font-semibold text-emerald-400 w-1/4">Dimension / Feature</th>
                      {comparison.headers?.map((h, i) => (
                        <th key={h.docId || i} className="p-3 font-semibold text-slate-200 border-l border-slate-800">
                          {h.title}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-slate-800 bg-slate-900/50">
                    {comparison.rows?.map((row, rIdx) => (
                      <tr key={rIdx} className="hover:bg-slate-900/80 transition-colors">
                        <td className="p-3 font-bold text-slate-300 bg-slate-950/40">{row.feature}</td>
                        {comparison.headers?.map((h, hIdx) => (
                          <td key={hIdx} className="p-3 text-slate-400 border-l border-slate-800 leading-relaxed">
